...
jQuery.fn.highlight = function(words, settings) {
	...
	function wordsHighlight(node, words) {
		if (node.nodeType == 3) {
			...
			jQuery(node).replaceWith(str.replace(regexp, '<span class="' + highlight_settings.className + '" data-text="$1">$1</span>'));
	...
	return this.each(function() {
		...
		$('span.highlight').mouseover(function(){
			$(this).removeClass('highlight').text($(this).data('text'))
		}).mouseout(function(){
			$(this).addClass('highlight').text($(this).data('text'))
		})
	});
};
